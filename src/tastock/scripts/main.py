...
class Assistant:
            ...
            try:
                ...
                if merged_csv_filepath and os.path.exists(merged_csv_filepath):
                    ...
                    Assistant.calculate_portfolio_performance_from_csv(
                        ...
                        output_dir=current_portfolio_target_dir,
                        metrics_filename_prefix=f"perf_{portfolio_name}",
    ...
    def fetch_portfolios_data_and_calculate_performance_to_save_to_csv(period=10, portfolios=PORTFOLIOS):
        ...
        all_portfolios_data = Assistant.fetch_portfolios_data_and_save_to_csv(
            portfolios_map=portfolios,
            period=period,
            base_output_dir=DATA_DIR + "/historical_data/" + Helpers.name_today_datetime(),
            use_sub_dir_for_portfolio_folders=True,
            use_sub_dir_for_timestamp_folders=False,
    ...
    def calculate_portfolio_performance_from_csv(
        ...
        metrics_data = portfolio_obj.get_performance_metrics()
        ...
        if not metrics_df.empty:
            output_filename = metrics_filename_prefix
        ...
        for symbol in symbols_list:
            ...
            try:
                ...
                all_symbols_intrinsic_data.append({
                    'symbol': symbol,
                    'graham_intrinsic_value': intrinsic_value if intrinsic_value is not None and pd.notna(intrinsic_value) else np.nan
                })
            except Exception as e:
...
if __name__ == "__main__":
    # Fetch historical data for the LongTerm portfolio, calculate performance metrics,
    # and save results to CSV files
    Assistant.fetch_portfolios_data_and_calculate_performance_to_save_to_csv(
        period=10, 
        portfolios={"LongTerm": PORTFOLIOS['LongTerm']}
    )